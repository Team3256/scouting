...
import { and, desc, eq, isNull, schema } from "@acme/db";
...
export const attendanceRouter = createTRPCRouter({
  registerUser: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      await ctx.db
        .insert(schema.attendanceKeys)
        .values({ uid: input.uid, currentlyAttending: false });
    }),
  checkIn: publicProcedure
    .input(z.object({ uid: z.string() }))
    .mutation(async ({ ctx, input }) => {
      const attendanceKey = await ctx.db.query.attendanceKeys.findFirst({
        where: eq(schema.attendanceKeys.uid, input.uid),
      });
      if (!attendanceKey) {
        throw new TRPCError({
          code: "NOT_FOUND",
          message: "Attendance key not found",
        });
      }
      if (attendanceKey.currentlyAttending) {
        throw new TRPCError({
      ...
      await ctx.db
        .update(schema.attendanceKeys)
        .set({ currentlyAttending: true })
        .where(eq(schema.attendanceKeys.uid, input.uid));
      await ctx.db.insert(schema.attendanceSessions).values({
        uid: input.uid,
        startTime: new Date(),
      });
    }),
  checkOut: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const lastUpdated = await ctx.db
        ...
        .where(
          and(
            eq(schema.attendanceSessions.uid, input.uid),
            isNull(schema.attendanceSessions.endTime),
          ),
        )
        .returning({ endTime: schema.attendanceSessions.endTime });
      if (!lastUpdated[0]?.endTime) {
        throw new TRPCError({
  ...
  assignName: publicProcedure
    .input(z.object({ uid: z.string(), name: z.string() }))
    .mutation(async ({ ctx, input }) => {
      return ctx.db.update(schema.attendanceKeys).set({ name: input.name });
    }),
  checkInOut: publicProcedure
    .input(z.object({ uid: z.string() }))
    .mutation(async ({ ctx, input }) => {
      const attendanceKey = await ctx.db.query.attendanceKeys.findFirst({
        where: eq(schema.attendanceKeys.uid, input.uid),
